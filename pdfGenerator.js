...
async function generatePDF(song) {
    return new Promise((resolve, reject) => {
        try {
            ...
            renderInWordStyleColumns(doc, linesWithSpacing, leftColumnX, rightColumnX, columnWidth, pageWidth);
            
            // After rendering, add footers to all pages with correct page numbers
            addFootersToAllPages(doc, pageWidth, leftColumnX);
...
function addFootersToAllPages(doc, pageWidth, leftColumnX) {
    const range = doc.bufferedPageRange();
    const totalPages = range.count;
    
    console.log(`📄 Adding footers to ${totalPages} pages`);
    
    // Go through each page
    for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
        try {
            // Switch to the page (using range.start + pageIndex for correct indexing)
            ...
            doc.fontSize(8)
               .font('Helvetica')
               .text(`Generated by Discord Song Bot - 9/11/2025`, 
            ...
            // Add page number
            doc.text(`${pageIndex + 1}/${totalPages}`, 
                     leftColumnX + (pageWidth * 0.7), doc.page.height - 30, {
                       width: pageWidth * 0.3,
                       align: 'right'
                     });
            ...
            console.log(`✅ Added footer to page ${pageIndex + 1}/${totalPages}`);
            
        } catch (error) {
            console.error(`❌ Error adding footer to page ${pageIndex + 1}:`, error.message);
        }
    }
    
    console.log(`✅ Completed adding footers with page numbering format: 1/${totalPages}, 2/${totalPages}, etc.`);
}
